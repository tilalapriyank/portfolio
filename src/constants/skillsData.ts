...
import type { SkillSection } from "@/types/portfolio";

const pickIcon = (...candidates: string[]) =>
  candidates
    .map((name) => Si[name as keyof typeof Si] || Fa[name as keyof typeof Fa] || Md[name as keyof typeof Md] || Ri[name as keyof typeof Ri])
    .find(Boolean) || Fa.FaCode;

export const mySkills: SkillSection[] = [
  {
